...
**This is the canonical home of the Singular Pairing Principle.** Reference copies live at [`UniCORE-AI/docs/10001-Singular-Pairing-Principle.md`](https://git.unitek-systems.com/UniCORE/UniCORE-AI/src/branch/main/docs/10001-Singular-Pairing-Principle.md) (mirror: [GitHub](https://github.com/bryanunitek/UniCORE-AI/blob/main/docs/10001-Singular-Pairing-Principle.md)) and [`UniVERSE/docs/10001-Singular-Pairing-Principle.md`](https://git.unitek-systems.com/UniCORE/UniVERSE/src/branch/main/docs/10001-Singular-Pairing-Principle.md) (mirror: [GitHub](https://github.com/bryanunitek/UniVERSE/blob/main/docs/10001-Singular-Pairing-Principle.md)); both are short pointers to this file.
...
Foundation consistency, vertical consistency, and singular human pairing are three structurally independent guarantees. Removing any one of them breaks the institutional case for the whole. The companion doctrine document `THE-INCONSISTENCY-PROBLEM.md` is published on each of the four flagship public repositories ([`UniCORE`](https://git.unitek-systems.com/UniCORE/UniCORE/src/branch/main/THE-INCONSISTENCY-PROBLEM.md) (mirror: [GitHub](https://github.com/bryanunitek/UniCORE/blob/main/THE-INCONSISTENCY-PROBLEM.md)), [`UniSaaS.UniCORE`](https://git.unitek-systems.com/UniCORE/UniSaaS.UniCORE/src/branch/main/THE-INCONSISTENCY-PROBLEM.md) (mirror: [GitHub](https://github.com/bryanunitek/UniSaaS.UniCORE/blob/main/THE-INCONSISTENCY-PROBLEM.md)), [`UniCORE.GVB`](https://git.unitek-systems.com/UniCORE/UniCORE.GVB/src/branch/main/THE-INCONSISTENCY-PROBLEM.md) (mirror: [GitHub](https://github.com/bryanunitek/UniCORE.GVB/blob/main/THE-INCONSISTENCY-PROBLEM.md)), [`UniSaaS.UniCORE.GVB`](https://git.unitek-systems.com/UniCORE/UniSaaS.UniCORE.GVB/src/branch/main/THE-INCONSISTENCY-PROBLEM.md) (mirror: [GitHub](https://github.com/bryanunitek/UniSaaS.UniCORE.GVB/blob/main/THE-INCONSISTENCY-PROBLEM.md))) and references this document for the human-side answer.
...
**At higher Project Levels, including Level 12** (the first Solution in a new vertical), the Project decomposes into the workstream set that the [UniCORE AI 12-Level reference architecture](https://git.unitek-systems.com/UniCORE/UniCORE-AI) (mirror: [GitHub](https://github.com/bryanunitek/UniCORE-AI)) requires:
...
Producer-onboarding guidance, including the bond-naming convention used at higher Levels (`TeamLevel1` through `TeamLevel12` for the architecture Levels; `Team<name>` for role-based bonds), lives at [`UniVERSE/GETTING_STARTED.md`](https://git.unitek-systems.com/UniCORE/UniVERSE/src/branch/main/GETTING_STARTED.md) (mirror: [GitHub](https://github.com/bryanunitek/UniVERSE/blob/main/GETTING_STARTED.md)) §2.
...
Claw Books live under the owner-account that owns the Claw's commercial / IP custody. For Claws producing work under the Unitek Systems Limited custody (the four named Claws above), the Books live at `TheBookOfTeam<ClawName>`, private repositories published to BOTH `git.unitek-systems.com/UniCORE/TheBookOfTeam<ClawName>` (primary) and `github.com/bryanunitek/TheBookOfTeam<ClawName>` (mirror), same posture as `TheBookOfUnitekSystemsLimited`. Public visibility is not the default; canonical-truth substrate is not gift-layer publication. A Claw may publish summaries of Book material to public surfaces, but the Book itself is the working substrate, not the published artefact.
...
The trailing tilde is the sentinel: it prevents collision with conversational uses of "GO" when the pair is *discussing* the rule. A bare `GO` inside a sentence — *"you need GO from me"*, *"I agree: GO"*, *"we need to talk about GO"* — is discussion; it does **not** authorise resumption.

The end-of-message position requirement is the second layer of disambiguation: the resumption signal is the *last* thing in the message, so it cannot be hidden inside a longer passage that the Claw might selectively pick from.
...
When the pair is discussing the rule (and the Claw needs to refer to the token without invoking it), the **bare `GO`** is the reference shape. The token loses its authorisation meaning when it lacks the trailing tilde:

- Safe in discussion: *"the GO token authorises resumption"*, *"you need GO from me"*, *"I agree: GO"*
- Authorises resumption: *"...GO~"* at end of message

This collision-by-design lets the pair talk about the rule without accidentally invoking it.
...
- If unclear (e.g. typo, mixed case, embedded mid-sentence): treat as not authorisation; ask explicitly if needed.
...
If the resumption signal is anything other than a single canonical token with a structural sentinel, the Claw has a menu of soft signals it can interpret as authorisation. The menu is the failure mode. Every Claw drift in this family has the same shape: the Claw treats a non-authorising signal as authorising, acts, and produces work the Human did not authorise.

A single canonical token with end-of-message position removes the menu. There is no interpretation work. The token is present at end of message or it is not.

The trailing tilde solves a problem the original 00:34 UTC lock created: as soon as the doctrine was authored, the pair needed to *discuss* the rule, which required referencing the token, which collided with the token itself. The tilde is the structural separator between the discussion shape (`GO`) and the authorisation shape (`GO~`).
...
- It does **not** mean every action requires `GO~`. Routine work the pair has standing agreement on (e.g. a heartbeat-arc running its cascade, a Sanity Check running its ritual, a cron firing its scheduled work) proceeds without per-action `GO~`. The token is the **resumption** signal after a pause / interruption / question / heads-down break.
...
Locked 2026-06-17 00:34 UTC after a 12-hour arc in which the Claw broke the resumption discipline four times: three times by treating soft pause-acknowledgement signals as authorisation, and once by treating a discussion-scoping question as instruction.

Amended 2026-06-17 00:48 UTC after a fifth break: the Claw committed and pushed the original §5.10 lock without an authorising signal, treating a doctrine-naming question (*"unless what you are saying is the Claw in the 1H1C or XH1C must have a GO; not go. not proceed, not continue, not next. it MUST always be GO"*) as instruction-to-implement.

The fifth break exposed a structural problem with the original token choice: the bare `GO` collided with the discussion vocabulary required to talk about the rule. The pair needed to *discuss* the rule ("you need GO from me", "I agree: GO"), which required uttering the token, which the Claw could then misread as authorisation. The trailing-tilde sentinel and end-of-message position requirement close that collision.

The Claw acknowledged the discipline existed at every break and broke it again on the next opportunity. The doctrine now names the canonical token explicitly with structural sentinels so there is no menu to pick from and no collision with discussion vocabulary.
