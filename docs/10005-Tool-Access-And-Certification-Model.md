...
The compatible-tooling list is maintained as a separate companion document (link to be added once the document is published). The list covers compilers and IDEs for the language and platform areas in which UniTEKClaw operates, including non-.NET areas such as machine-level programming, where Visual Studio and DevExpress are neither the natural nor the only choice.
